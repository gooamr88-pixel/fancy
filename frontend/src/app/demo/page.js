...
export default function DemoPage() {
  const [phase, setPhase] = useState('closed');  // closed → opening → invite → rsvp → done
  ...
  const handleOpen = () => {
    ...
    setPhase('opening');
    // After flap opens, slide the card out
    setTimeout(() => setPhase('sliding'), 900);
    // After card slides, show full invitation
  ...
  return (
    <div className="demo">
      ...
      <div className="ambient">
        {Array.from({ length: 15 }, (_, i) => (
          <div key={i} className="bokeh" style={{
            ...
            width: `${8+Math.random()*16}px`, height: `${8+Math.random()*16}px`,
            animationDelay: `${Math.random()*6}s`, animationDuration: `${7+Math.random()*6}s`,
          }} />
        ))}
      </div>
      ...
      {/* ═══ ENVELOPE + CARD PHASES (closed / opening / sliding) ═══ */}
      {(phase === 'closed' || phase === 'opening' || phase === 'sliding') && (
        <div className="envelope-scene">
          <div className={`envelope ${phase}`} onClick={handleOpen}>
            
            {/* ── Envelope Back (bottom layer) ── */}
            <div className="env-back">
              {/* The card that slides up */}
              <div className={`env-card-inner ${phase === 'sliding' ? 'card-slide-up' : ''}`}>
                <div className="env-card-content">
                  <div className="ecc-ornament">✦</div>
                  <p className="ecc-label">YOU'RE INVITED</p>
                  <h2 className="ecc-names">
                    <span className="ecc-script">Sophia</span>
                    <span className="ecc-amp">&</span>
                    <span className="ecc-script">Alexander</span>
                  </h2>
                  <div className="ecc-line">
                    <span/><span className="ecc-diamond">◆</span><span/>
                  </div>
                  <p className="ecc-date">September 20, 2025</p>
                  <p className="ecc-venue">The Grand Rosewood Estate</p>
                </div>
              </div>
            </div>

            {/* ── Envelope Front (the V-fold that sits on top) ── */}
            <div className="env-front">
              <div className="env-front-pattern" />
              {/* Gold border trim */}
              <div className="env-front-border" />
              {/* Monogram */}
              ...
              <p className="env-date-label">09 · 20 · 2025</p>
            </div>

            {/* ── Envelope Flap (the triangle lid) ── */}
            <div className={`env-flap ${phase === 'opening' || phase === 'sliding' ? 'flap-open' : ''}`}>
          ...
          {/* Hint text */}
          <p className={`hint ${phase !== 'closed' ? 'hint-hidden' : ''}`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ...
          <div className="done-card">
            ...
            {attending ? (
              <>
                ...
                <div className="done-qr"><div className="qr-box"><svg width="44" height="44" viewBox="0 0 44 44"><rect x="2" y="2" width="12" height="12" rx="2" stroke="#B8944F" strokeWidth="1.2" fill="none"/><rect x="4" y="4" width="8" height="8" rx="1" fill="#B8944F" opacity=".2"/><rect x="30" y="2" width="12" height="12" rx="2" stroke="#B8944F" strokeWidth="1.2" fill="none"/><rect x="32" y="4" width="8" height="8" rx="1" fill="#B8944F" opacity=".2"/><rect x="2" y="30" width="12" height="12" rx="2" stroke="#B8944F" strokeWidth="1.2" fill="none"/><rect x="4" y="32" width="8" height="8" rx="1" fill="#B8944F" opacity=".2"/><rect x="18" y="18" width="8" height="8" rx="1" fill="#B8944F" opacity=".15"/><rect x="32" y="32" width="8" height="8" rx="1" fill="#B8944F" opacity=".15"/></svg></div><small>Digital Check-In Pass</small></div>
              </>
            ) : (
              <>
            ...
            <div className="done-actions">
              <button onClick={() => { setPhase('closed'); setGuestName(''); setAttending(null); setGuestCount(1); setMealChoice(''); setConfetti([]); }}
      ...
      <style jsx>{`
        /* ═══ BASE ═══ */
        .demo { min-height:100vh; background:#0A0A0C; position:relative; overflow-x:hidden; font-family:var(--font-sans); }
        .ambient { position:fixed; inset:0; pointer-events:none; z-index:0; }
        .bokeh { position:absolute; border-radius:50%; background:radial-gradient(circle, rgba(184,148,79,.4) 0%, transparent 70%); opacity:.12; animation:bokehDrift ease-in-out infinite alternate; }
        @keyframes bokehDrift { 0%{transform:translateY(0) scale(1)} 100%{transform:translateY(-20px) scale(1.15)} }

        .back-btn { position:fixed; top:20px; left:20px; z-index:100; display:flex; align-items:center; gap:6px; color:rgba(255,255,255,.4); font-size:12px; font-weight:600; text-decoration:none; letter-spacing:.05em; transition:color .3s; }
        .back-btn:hover { color:#D7BE80; }

        /* ═══ ENVELOPE SCENE ═══ */
        .envelope-scene { min-height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:28px; position:relative; z-index:1; animation:fadeIn .8s ease both; }
        .envelope { width:380px; height:260px; position:relative; cursor:pointer; perspective:1200px; transition:transform .4s ease; }
        .envelope.closed:hover { transform:scale(1.02) translateY(-4px); }
        .envelope.opening, .envelope.sliding { cursor:default; }

        /* ── Envelope Back ── */
        .env-back {
          position:absolute; inset:0;
          background:linear-gradient(160deg, #F5EFE4 0%, #EDE5D5 100%);
          border-radius:6px;
          box-shadow: 0 24px 60px rgba(0,0,0,.45), 0 0 0 1px rgba(215,190,128,.15);
          overflow:hidden;
        }
        .env-back::after {
          content:''; position:absolute; inset:6px;
          border:1px solid rgba(215,190,128,.12);
        ...
        .env-card-inner {
          ...
          background:linear-gradient(170deg, #FFFDF9 0%, #FBF8F2 100%);
          border:1px solid rgba(215,190,128,.18);
        ...
        .env-card-content { text-align:center; padding:20px; }
        .ecc-ornament { color:#D7BE80; font-size:12px; letter-spacing:6px; margin-bottom:8px; }
        .ecc-label { font-size:8px; font-weight:800; letter-spacing:.35em; text-transform:uppercase; color:#B8944F; margin:0 0 10px; font-family:var(--font-sans); }
        .ecc-names { margin:0 0 6px; line-height:1; }
        .ecc-script { font-family:var(--font-script); font-size:32px; color:#191B1E; display:block; }
        .ecc-amp { font-family:var(--font-serif); font-size:14px; color:#B8944F; display:block; font-style:italic; margin:2px 0; }
        .ecc-line { display:flex; align-items:center; justify-content:center; gap:6px; margin:10px 0; }
        .ecc-line span:first-child, .ecc-line span:last-child { width:28px; height:1px; background:linear-gradient(90deg, transparent, #D7BE80, transparent); }
        .ecc-diamond { color:#D7BE80; font-size:6px; }
        .ecc-date { font-family:var(--font-serif); font-size:12px; color:#77736A; margin:0 0 2px; font-style:italic; }
        .ecc-venue { font-family:var(--font-serif); font-size:11px; color:#9A9590; margin:0; }

        /* ── Envelope Front (V-shaped bottom cover) ── */
        .env-front {
        ...
        .env-front-pattern {
          ...
          background:url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23B8944F'%3E%3Cpath d='M20 0l3 10h10l-8 6 3 10-8-6-8 6 3-10-8-6h10z' fill-opacity='.3'/%3E%3C/g%3E%3C/svg%3E");
          pointer-events:none;
        }
        .env-front-border {
          position:absolute; inset:0;
          clip-path: polygon(0 30%, 50% 0, 100% 30%, 100% 100%, 0 100%);
          border-bottom:2px solid rgba(215,190,128,.2);
          pointer-events:none;
        }
        .env-monogram {
          display:flex; align-items:baseline; gap:4px; z-index:1;
        }
        .mono-s, .mono-a { font-family:var(--font-serif); font-size:22px; font-weight:700; color:#8A7D6B; }
        .mono-amp { font-family:var(--font-script); font-size:18px; color:#B8944F; }
        .env-date-label { font-size:9px; font-weight:600; letter-spacing:.3em; color:#9A9590; margin-top:6px; font-family:var(--font-sans); z-index:1; }
        ...
        .flap-seal {
          ...
          background:radial-gradient(circle at 40% 35%, #C94A3F 0%, #7B2D28 80%);
          display:flex; align-items:center; justify-content:center;
          box-shadow:0 3px 8px rgba(123,45,40,.3), inset 0 1px 2px rgba(255,255,255,.12);
          transform:translateY(12px);
        }
        .flap-seal span {
          font-family:var(--font-serif); font-size:18px; font-weight:700;
          color:rgba(255,255,255,.85); text-shadow:0 1px 2px rgba(0,0,0,.2);
        }
        ...
        /* ═══ INVITATION PHASE (reused from before) ═══ */
        .invite-phase { min-height:100vh; display:flex; justify-content:center; padding:40px 20px; animation:invReveal 1s cubic-bezier(.16,1,.3,1) both; background:linear-gradient(180deg, #0A0A0C 0%, #141316 50%, #0A0A0C 100%); position:relative; z-index:1; }
