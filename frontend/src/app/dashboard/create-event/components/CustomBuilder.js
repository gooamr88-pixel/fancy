...
const labelStyle = {
  fontSize: 'var(--fx-micro)', fontWeight: 700, color: '#77736A', textTransform: 'uppercase',
...
function ColorField({ label, value, onChange }) {
  return (
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, border: '1px solid #E8E2D6', borderRadius: 8, padding: '5px 8px', background: '#fff' }}>
          ...
          {FONTS.map(f => {
            ...
            return (
              <button key={f.key} onClick={() => onChange({ headingFont: f.key })}
                style={{
                  flex: '1 1 calc(33.333% - 6px)', minWidth: 64, padding: '8px 6px', minHeight: 'var(--fx-touch)', borderRadius: 10, cursor: 'pointer',
                ...
                <span style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--fx-micro)', fontWeight: 600, color: active ? '#B8944F' : '#77736A' }}>{f.label}</span>
              </button>
            );
          })}
        </div>
