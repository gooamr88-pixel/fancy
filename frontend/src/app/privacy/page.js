...
const sections = [
  ...
  },
  {
    ...
    content: [
      ...
      "**SMS Consent Is Independent and Optional:** The SMS consent checkbox is unchecked by default and is never pre-selected. Agreeing to receive text messages is separate from, and not conditioned on, acceptance of this Privacy Policy or our Terms of Service, and you are not required to accept either one in order to opt in to SMS. Equally, opting in to SMS is not a condition of using Fancy RSVP, of submitting an RSVP, or of attending an event: a guest may supply a phone number for the host's use while leaving the SMS box unchecked, and the RSVP is accepted exactly as it would be otherwise. **The phone number itself is optional on every form we operate.** No guest is required to provide a mobile number in order to register, RSVP, attend, or use any feature, and the consent checkbox is only ever shown to a guest who has chosen to enter one — so a guest can remain entirely outside the text-messaging programme without giving anything up. A number whose owner was shown the checkbox and left it unchecked is recorded as a declined consent and is excluded from every outbound message at the point of sending. Consent is also never collected by, bundled into, or inferred from any other agreement or action: neither accepting this Privacy Policy, nor accepting our Terms of Service, nor creating an account, nor submitting an RSVP, nor providing a phone number, constitutes consent to receive text messages, and no \"continue\" or \"submit\" button anywhere on our platform collects SMS consent as a side effect.",
