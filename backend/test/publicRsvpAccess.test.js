...
test('H1: getGuestById never returns email / phone / notes', async () => {
  ...
  mock.setResolver(({ table }) => {
    if (table === 'rsvp_parties') return { data: {
      id: 'r1', label: 'Alex', response: 'yes',
      events: { slug: 'wedding', is_paid: true, status: 'active', event_date: '2026-06-01T00:00:00Z' },
      seating_assignments: [{ tables: { table_name: 'Table 1' } }],
      guests: [{ id: 'g1', is_primary_contact: true, email: 'alex@x.com', phone: '+15550000000', notes: 'private note' }]
    } };
    return {};
  });
  const { res } = await invoke(getGuestById, mockReq({ params: { guestId: 'r1' } }));
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.data.guest.guest_name, 'Alex');
  assert.equal(res.body.data.guest.table_name, 'Table 1');
  assert.equal(res.body.data.guest.email, undefined, 'email must NOT be exposed');
  assert.equal(res.body.data.guest.phone, undefined, 'phone must NOT be exposed');
  assert.equal(res.body.data.guest.notes, undefined, 'notes must NOT be exposed');
});

test('getGuestById 404s for a non-active event (no leak from draft/paused events)', async () => {
  mock.setResolver(({ table }) => table === 'rsvp_parties' ? { data: {
    id: 'r1', label: 'Alex', response: 'yes',
    events: { slug: 'w', is_paid: true, status: 'draft', event_date: '2026-06-01T00:00:00Z' },
    seating_assignments: [],
    guests: [{ id: 'g1' }]
  } } : {});
  const { res } = await invoke(getGuestById, mockReq({ params: { guestId: 'r1' } }));
...
test('H1: a 1-char query returns nothing and issues NO guest query', async () => {
  ...
  assert.equal(mock.calls.some(c => c.table === 'rsvp_parties'), false, 'must not scan guests for a 1-char term');
});
...
test('search returns names + response (never contact info) for a live event', async () => {
  mock.setResolver(({ table }) => {
    ...
    if (table === 'rsvp_parties') return { data: [{
      id: 'r1', label: 'Alex Smith', response: 'yes',
      guests: [{ id: 'g1', is_primary_contact: true, email: 'alex@x.com', phone: '+15550000000' }]
    }] };
    return {};
  });
  const { res } = await invoke(searchPublicGuests, mockReq({ params: { slug: 'w' }, query: { query: 'alex' } }));
  ...
  assert.equal(res.body.data.results[0].guestName, 'Alex Smith');
  assert.equal(res.body.data.results[0].email, undefined);
  assert.equal(res.body.data.results[0].phone, undefined);
});
