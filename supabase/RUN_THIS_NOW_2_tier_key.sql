-- ════════════════════════════════════════════════════════════════════════
-- عاجل — الدفع واقف. شغّل ده.
--
-- (النسخة دي مصلَّحة. اللي بعتهولك قبل كده وقع بـ 42803 — الغلط كان في
--  الـmigration الأصلي نفسه، وده بالظبط سبب إنه عمره ما اتطبق: مكسور.)
--
-- الفحص أثبت إن `events.tier_key` مش موجود. الملف ده بيضيف ٧ أعمدة على
-- ٣ جداول ويملاها.
--
-- الأثر الحالي: زرار الدفع بيرجّع «Event not found» — مش لأن المناسبة مش
-- موجودة، لكن لأن الاستعلام بيسأل عن عمود مش موجود، فPostgREST يرفض
-- الاستعلام كله، والكود يترجم الرفض لـ404.
--
-- آمن: كل الأعمدة IF NOT EXISTS، كل تعبئة محمية بـ tier_key IS NULL،
-- وكله جوه transaction واحدة. يتشغّل أكتر من مرة من غير ضرر.
--
-- مولّد من supabase/migrations/20260818000000_tier_identity.sql
-- ════════════════════════════════════════════════════════════════════════
...
WITH checks(sort_key, col, present) AS (
  VALUES
    (1, 'events.tier_key',                 EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='events'         AND column_name='tier_key')),
    (2, 'events.tier_features',            EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='events'         AND column_name='tier_features')),
    (3, 'events.tier_price_cents',         EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='events'         AND column_name='tier_price_cents')),
    (4, 'event_payments.tier_key',         EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='event_payments' AND column_name='tier_key')),
    (5, 'event_payments.tier_features',    EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='event_payments' AND column_name='tier_features')),
    (6, 'event_payments.tier_price_cents', EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='event_payments' AND column_name='tier_price_cents')),
    (7, 'promo_codes.tier_key',            EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='promo_codes'    AND column_name='tier_key'))
)
SELECT CASE WHEN present THEN 'applied' ELSE '** MISSING **' END AS status, col
FROM checks ORDER BY present, sort_key;

-- المفروض 0. لو طلع رقم، يبقى فيه مناسبات مدفوعة أسماء باقاتها مش مطابقة
-- لأي باقة في الإعدادات الحالية — ابعتهولي.
SELECT 'PAID EVENTS STILL WITHOUT A TIER KEY' AS question, count(*)::text AS answer
FROM public.events
WHERE is_paid = true AND tier_name IS NOT NULL AND tier_key IS NULL;

-- والمفاتيح اللي اتولدت للباقات — المفروض كل واحدة ليها key فريد.
SELECT t.tier->>'name' AS tier_name, t.tier->>'key' AS tier_key
FROM super_admin_config sac, LATERAL jsonb_array_elements(sac.pricing_tiers) AS t(tier)
WHERE sac.id = '00000000-0000-0000-0000-000000000000';
