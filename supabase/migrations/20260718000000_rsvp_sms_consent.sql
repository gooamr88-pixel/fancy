-- ════════════════════════════════════════════════════════════════════════
-- SMS opt-in consent capture (Twilio A2P 10DLC / TCPA compliance)
-- ────────────────────────────────────────────────────────────────────────
-- The public RSVP wizard now shows an explicit SMS consent checkbox next to
-- the phone number field. This migration adds the columns to persist that
-- choice as a timestamped, auditable record — required evidence for A2P
-- 10DLC campaign registration (Twilio reviewers ask for proof of a real,
-- documented opt-in event, not just a policy page).
--
-- sms_consent reflects the CURRENT opt-in status (a resubmission that
-- unchecks the box is a genuine revocation and must be honored — unlike
-- `side`, this is never preserved via COALESCE on omission).
-- sms_consent_at is stamped to now() on every write through this RPC, so a
-- "false" value at a point in time is itself a meaningful compliance record
-- ("guest explicitly declined at this timestamp").
-- ════════════════════════════════════════════════════════════════════════
...
-- submit_rsvp_v2 gains a trailing p_sms_consent (signature change — drop the
-- old 14-arg overload first, same convention used throughout this repo).
