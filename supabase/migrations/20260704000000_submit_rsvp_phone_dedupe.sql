-- ════════════════════════════════════════════════════════════════════════
-- submit_rsvp() — duplicate-PHONE guard on the INSERT path
-- ────────────────────────────────────────────────────────────────────────
-- Supersedes 20260701000000_submit_rsvp_caps_limits.sql. Single change:
--
--   DEDUPE  The public RSVP form requires a phone number but treats email as
--           optional. A returning guest who opens the public link WITHOUT an
--           invitation token (no p_rsvp_id) and skips the name search would hit
--           the INSERT path and create a SECOND row — the email guard can't stop
--           it when no email was given. We now also reject an insert when an
--           active (non-declined) RSVP with the same phone already exists for the
--           event, mirroring the existing email guard. p_phone arrives already
--           normalised to E.164 from the controller, matching how imported /
--           organizer-added guests are stored, so the equality match is exact.
--
...
BEGIN
  ...
  IF p_rsvp_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- INSERT path: duplicate-email guard + duplicate-phone guard + unique-index backstop.
    ...
    -- DEDUPE: the public form requires a phone, so a returning guest who skips the
    -- name search (no p_rsvp_id) and re-submits would otherwise create a second
    -- row — the email guard above can't catch it when no email was supplied. Reject
    -- when an active (non-declined) RSVP already carries this phone. p_phone is
    -- already E.164-normalised by the caller, matching the stored format.
    IF p_phone IS NOT NULL AND btrim(p_phone) <> '' THEN
      IF EXISTS (SELECT 1 FROM rsvps WHERE event_id = v_event.id AND phone = p_phone AND response <> 'no') THEN
        RETURN jsonb_build_object('success', false, 'code', 'DUPLICATE_RSVP',
