-- ════════════════════════════════════════════════════════════════════════
-- Bug 10 fix: Duplicate Reservations — auto-merge instead of rejecting
-- ────────────────────────────────────────────────────────────────────────
-- When a guest submits from the public link with an email/phone that
-- already has an RSVP for this event, instead of returning DUPLICATE_RSVP
-- the system now auto-updates (merges into) the existing party record.
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  ELSE
    ...
    -- switch to the UPDATE path (auto-merge).

    -- INSERT path: duplicate-email auto-merge
    IF v_norm_email IS NOT NULL THEN
      SELECT p.id INTO v_party_id FROM guests g JOIN rsvp_parties p ON p.id = g.party_id
