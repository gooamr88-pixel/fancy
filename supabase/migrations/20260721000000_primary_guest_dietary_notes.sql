-- ════════════════════════════════════════════════════════════════════════
-- Dietary Restrictions & Allergies — add support for the primary guest
-- ────────────────────────────────────────────────────────────────────────
-- The guests.dietary_notes column has always been populated for companions
-- (see the additional-guests loop below), but every INSERT that creates the
-- PRIMARY guest's row never set it — there was no parameter to carry it.
-- The public RSVP wizard's dietary field only ever existed on the companion
-- cards, so the primary guest (the person actually filling out the form)
-- had no way to record dietary restrictions at all — an inconsistency with
-- Meal Selection, which already exists symmetrically for the primary guest
-- and every companion.
--
-- Adds a trailing p_primary_dietary_notes param (defaulted, so this is a
-- purely additive signature change — PostgREST calls by name, and existing
-- callers omitting it are unaffected) and threads it into the primary
-- guest's three INSERT sites, gated on p_response = 'yes' exactly like
-- p_primary_meal already is.
-- ════════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.submit_rsvp_v2(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, JSONB, JSONB, TEXT, TEXT, TEXT, BOOLEAN);
