-- ════════════════════════════════════════════════════════════════════════
-- Fix: submit_rsvp_v2 concurrency + lock-bypass bugs
-- ────────────────────────────────────────────────────────────────────────
-- 1. The guest-cap check (BIZ-1) was check-then-act with no lock, so two
--    simultaneous public RSVP submissions could both pass the cap check
--    and both commit, overshooting tier_max_guests. Fixed by taking a
--    per-event transactional advisory lock before the cap check, held
--    through the write (released automatically on commit/rollback).
-- 2. The duplicate-email and duplicate-phone auto-merge branches (added in
--    20260706000000_submit_rsvp_auto_merge.sql) updated the matched party
--    without checking whether its response was already locked in (yes/
--    no/maybe with allow_guest_edits disabled). This let anyone who knew
--    or guessed a guest's email/phone silently overwrite their locked
--    RSVP via the public, unauthenticated endpoint. Fixed by applying the
--    same lock check used on the explicit p_party_id path.
-- ════════════════════════════════════════════════════════════════════════
