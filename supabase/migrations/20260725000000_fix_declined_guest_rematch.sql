-- ════════════════════════════════════════════════════════════════════════
-- FIX: declined guests were unmatchable on resubmission, causing silent
-- duplicate rsvp_parties rows (or, when contact info matched, a false
-- DUPLICATE_RSVP rejection even with allow_guest_edits enabled).
--
-- Root cause: the auto-merge lookup in submit_rsvp_v2's INSERT path
-- (email match + phone match) carried `AND p.response <> 'no'`, excluding
-- any previously-declined party from being found again. That exclusion
-- was written for the OLD `rsvps` table (20260610100000_security_hardening),
-- whose unique index was itself response-scoped (`WHERE response != 'no'`) —
-- declines were deliberately allowed to collide freely because the DB
-- schema didn't enforce uniqueness against them either.
--
-- 20260705000000_guest_experience_rebuild replaced that with the current
-- `guests` table, whose unique indexes (idx_guests_event_email_unique /
-- idx_guests_event_phone_unique) are UNCONDITIONAL — not response-scoped.
-- The `<> 'no'` exclusion in submit_rsvp_v2 was carried forward unchanged
-- across every migration since, so it no longer matches the policy its own
-- unique indexes already enforce. Concretely, this let a guest whose
-- device-remembered party_id was lost (private browsing, in-app browser
-- storage partitioning, etc.) submit a "fresh" form after declining:
--   - same email/phone as before  -> auto-merge lookup skips the match ->
--     falls to INSERT -> collides with the unconditional unique index ->
--     rejected as DUPLICATE_RSVP even when the host allows guest edits.
--   - blank/different contact info (declines require neither) -> no match
--     attempted at all -> INSERT succeeds -> a second, orphaned
--     rsvp_parties row is silently created for the same person.
--
-- Fix: drop `AND p.response <> 'no'` from both lookups. A previously-
-- declined guest is now matched like any other and routed through the
-- SAME allow_guest_edits gate already used for 'yes'/'maybe' (line just
-- below each lookup: `IF v_existing_resp IN ('yes','no','maybe') AND NOT
-- allow_guest_edits THEN DUPLICATE_RSVP`), so behaviour is now consistent
-- regardless of what the guest's last response was.
--
...
BEGIN
  ...
  LOOP
    ...
    -- Guest-scoped fields are also asked of each companion (attending only).
    IF v_field.scope = 'guest' AND p_response = 'yes' AND v_party_size > 1 THEN
      FOR i IN 0..(v_party_size - 2) LOOP
        v_g := p_additional_guests -> i;
