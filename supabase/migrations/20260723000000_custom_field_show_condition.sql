-- ════════════════════════════════════════════════════════════════════════
-- CUSTOM FIELD SHOW CONDITION ("Always Show" vs "If Attending")
--
-- The Form Builder has always offered an "Always Show / If Attending" toggle per
-- custom question, but it was never persisted (no column) nor honored — both
-- guest RSVP surfaces only showed custom questions when attending, and
-- submit_rsvp_v2 only stored/validated custom answers for p_response = 'yes'.
--
-- This migration adds the `condition` column (DEFAULT 'attending', so every
-- existing field keeps today's EXACT behavior — zero regression) and updates
-- submit_rsvp_v2 so 'always' questions are stored + required-validated for every
-- response, while 'attending' questions stay gated behind attendance.
--
-- Companions only exist for p_response = 'yes' (see step 3, v_party_size), so the
-- 'always' path only ever concerns the PRIMARY guest when declining/tentative.
-- ════════════════════════════════════════════════════════════════════════

ALTER TABLE public.custom_form_fields
  ADD COLUMN IF NOT EXISTS condition TEXT NOT NULL DEFAULT 'attending';

ALTER TABLE public.custom_form_fields
  DROP CONSTRAINT IF EXISTS custom_form_fields_condition_check;
ALTER TABLE public.custom_form_fields
  ADD CONSTRAINT custom_form_fields_condition_check CHECK (condition IN ('always', 'attending'));
