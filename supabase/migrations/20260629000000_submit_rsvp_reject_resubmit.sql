-- ════════════════════════════════════════════════════════════════════════
-- submit_rsvp() — reject resubmission against an already-responded record
-- ────────────────────────────────────────────────────────────────────────
-- The public RSVP page now enforces a strict, state-aware flow: once a guest
-- has answered (yes / no / maybe) the form is hidden and an "already
-- registered" card is shown instead. This migration adds the matching
-- SERVER-SIDE backstop so a replayed/forged POST can't slip a second response
-- past the UI.
--
-- Rule: the UPDATE path (p_rsvp_id supplied) is only allowed while the target
-- record is still un-answered (response IS NULL or 'pending'). A record that
-- already holds 'yes' / 'no' / 'maybe' is locked and returns DUPLICATE_RSVP
-- (409) — the same code the controller already maps, so no controller change
-- is needed.
--
-- This does NOT affect the one-click email flow (respondViaToken) or the
-- organizer dashboard, which UPDATE rsvps directly and never call this RPC.
-- First-time responses for organizer-imported 'pending' invitations are
-- unaffected. Everything else in submit_rsvp() is unchanged from
...
DECLARE
  ...
  v_existing_resp   TEXT;      -- existing response on the update path (resubmission guard)
  v_found           BOOLEAN;
  v_rsvp_id         UUID;
...
BEGIN
  ...
  IF p_rsvp_id IS NOT NULL THEN
    ...
    -- Strict, state-aware lock: once a guest has answered, the record is closed
    -- to further public submissions. The UI shows an "already registered" card;
    -- this is the server-side backstop against a replayed/forged POST. First
